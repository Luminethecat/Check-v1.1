...
// 打卡机硬件需连接 Cpolar 映射后的后端地址
const BACKEND_PUBLIC_URL = 'https://xxxx.cpolar.top';
```
3. JWT 密钥
javascript
运行
...
核心功能模块
1. 身份认证
管理员登录：POST https://xxxx.cpolar.top/api/login
员工登录：POST https://xxxx.cpolar.top/api/user/login
管理员改密：POST https://xxxx.cpolar.top/api/admin/changePwd
员工改密：POST https://xxxx.cpolar.top/api/user/changePwd
2. 员工管理
查询员工列表：GET https://xxxx.cpolar.top/api/emp/list
新增 / 编辑 / 删除员工：POST/GET https://xxxx.cpolar.top/api/emp/xxx
重置员工密码：POST https://xxxx.cpolar.top/api/emp/resetPwd
3. 打卡记录
员工个人打卡记录：GET https://xxxx.cpolar.top/api/user/att/list
管理员全量打卡记录（分页）：GET https://xxxx.cpolar.top/api/att/list
远程打卡 / 设备指令：POST https://xxxx.cpolar.top/api/device/sendCmd
4. 考勤统计
员工年度统计：GET https://xxxx.cpolar.top/api/user/att/stat
全员工统计：GET https://xxxx.cpolar.top/api/att/getEmpAtt
Excel 报表导出：GET https://xxxx.cpolar.top/api/att/export
接口响应格式
所有接口统一返回 JSON 格式：
json
{
...
└── README.md       # 本说明文档
Cpolar 内网穿透注意事项
端口一致性：Cpolar 映射的端口（3000）必须和 app.js 中监听的端口一致；
服务保活：VS Code 关闭 / 终端退出会导致后端服务停止，需保持 VS Code 窗口和终端在线；
地址同步：Cpolar 隧道地址变化后，需同步修改：
打卡机硬件的后端接口地址；
前端页面的接口请求地址；
app.js 中 BACKEND_PUBLIC_URL 配置；
防火墙放行：确保本地电脑 3000 端口未被防火墙拦截，Cpolar 客户端能正常联网。
常见问题
Cpolar 隧道启动失败：
...
总结
核心流程：VS Code 运行后端 → Cpolar 映射 3000 端口 → 公网设备访问 Cpolar 地址；
关键要点：端口一致、服务保活、地址同步；
故障排查优先检查：后端是否启动、Cpolar 隧道是否在线、地址是否匹配。

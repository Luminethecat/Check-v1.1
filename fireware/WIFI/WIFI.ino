...
// ===================== 基础配置（首次烧录需修改）=====================
#define DEFAULT_WIFI_SSID "12345678"  // 初始2.4G WiFi账号
#define DEFAULT_WIFI_PWD  "12345678"   // 初始2.4G WiFi密码
 ...
 */
bool checkFrameValid(uint8_t* frame, uint8_t len) {
  if (len < 6) return false; // 最小帧长度：头(1)+类型(1)+长度(1)+CRC(2)+尾(1) = 6，数据可为0字节
  if (frame[0] != FRAME_HEAD || frame[len-1] != FRAME_TAIL) return false;
  uint8_t data_len = frame[2];
  uint8_t actual_data_len = len - 6; // 总长度 - 头(1)-类型(1)-长度(1)-CRC(2)-尾(1)
  ...
  while (Serial.available() > 0) {
    ...
    if (serial_buf_idx > 0) {
      ...
      if (serial_buf_idx < FRAME_BUF_LEN - 1) {
        ...
        // 检测到帧尾，校验并处理
        if (ch == FRAME_TAIL) {
          Serial.printf("检测到帧尾，尝试校验，当前缓冲区长度: %d\n", serial_buf_idx);
          Serial.printf("完整帧数据: ");
          for(int i = 0; i < serial_buf_idx; i++) {
          ...
          if (checkFrameValid(serial_buf, serial_buf_idx)) {
            ...
            Serial.printf("帧校验通过，类型: 0x%02X (%s), 数据长度: %d\n", type, 
              type==0x02?"CHECK_DATA":type==0x08?"USER_REGISTER":type==0x03?"TIME_REQ":"OTHER");
            Serial.printf("数据内容: ");
            for(int i = 0; i < len && i < 32; i++) {
              if(isprint(data[i])) Serial.printf("%c", data[i]);
              else Serial.printf("[%02X]", data[i]);
            }
            Serial.printf("\n");
            handleSTM32Frame(type, data, len);
            blinkLED(1, 200); // LED闪1次：收到有效帧
          } else {
            Serial.printf("帧校验失败，丢弃数据\n");
            // 输出详细校验失败信息
            if (serial_buf_idx >= 7) {
              uint8_t expected_len = serial_buf[2];
              uint8_t actual_data_len = serial_buf_idx - 6; // 头(1)+类型(1)+长度(1)+CRC(2)+尾(1)=6
              uint16_t crc_calc = CRC16_Modbus_Table(&serial_buf[3], expected_len);
              uint16_t crc_recv = (serial_buf[3+expected_len] << 8) | serial_buf[3+expected_len+1];
              Serial.printf("  期望数据长度: %d, 实际数据部分长度: %d\n", expected_len, actual_data_len);
              Serial.printf("  计算CRC: 0x%04X, 接收CRC: 0x%04X, 匹配: %s\n", 
                crc_calc, crc_recv, crc_calc==crc_recv?"YES":"NO");
            }
            Serial.printf("  完整缓冲区长度: %d, 内容: ", serial_buf_idx);
            for(int i = 0; i < serial_buf_idx && i < 32; i++) {
              Serial.printf("%02X ", serial_buf[i]);
            }
            Serial.printf("\n");
          }
        ...
        // 缓冲区溢出，重置
        Serial.println("缓冲区溢出，重置");
        serial_buf_idx = 0;
        memset(serial_buf, 0, FRAME_BUF_LEN);
      }
    } else if (ch == FRAME_HEAD) {
      // 检测到帧头，开始接收
      Serial.printf("检测到帧头: 0x%02X\n", ch);
      serial_buf_idx = 0;
      memset(serial_buf, 0, FRAME_BUF_LEN);
      serial_buf[serial_buf_idx++] = ch;
    } else {
      // 不是帧头，可能是在帧中间的字节或错误数据
      Serial.printf("非帧头字节: 0x%02X, 丢弃\n", ch);
    }
  }
}
...
void handleSTM32Frame(uint8_t type, uint8_t* data, uint8_t len) {
  ...
  if (type == TYPE_CHECK_DATA && len >= 25) {
    ...
    Serial.printf("上报MQTT数据: %s\n", json.c_str());
    Serial.printf("MQTT主题: %s\n", MQTT_TOPIC_PUB.c_str());
    Serial.printf("MQTT连接状态: %s\n", mqttClient.connected() ? "已连接" : "未连接");

    if (mqttClient.publish(MQTT_TOPIC_PUB.c_str(), json.c_str())) {
      Serial.println("MQTT发布成功");
    } else {
      Serial.println("MQTT发布失败");
      Serial.printf("MQTT状态码: %d\n", mqttClient.state());
    }
  }
  else if (type == TYPE_ADD_USER) {
    Serial.printf("收到TYPE_ADD_USER帧: len=%d\n", len);
    Serial.printf("用户信息数据: ");
    ...
    // 尝试解析用户信息
    char userData[128] = {0};
    memcpy(userData, data, (len < 127) ? len : 127);
    userData[127] = '\0';
    Serial.printf("用户信息字符串: %s\n", userData);

    // 帧转JSON并上报MQTT（后端入库）
    String json = frame2Json(type, data, len);
    Serial.printf("上报用户信息MQTT: %s\n", json.c_str());
    Serial.printf("MQTT主题: %s\n", MQTT_TOPIC_PUB.c_str());
    Serial.printf("MQTT连接状态: %s\n", mqttClient.connected() ? "已连接" : "未连接");

    if (mqttClient.publish(MQTT_TOPIC_PUB.c_str(), json.c_str())) {
      Serial.println("用户信息MQTT发布成功");
    } else {
      Serial.println("用户信息MQTT发布失败");
      Serial.printf("MQTT状态码: %d\n", mqttClient.state());
    }
  }
  else if (type == TYPE_USER_REGISTER) {
    Serial.printf("======= 收到TYPE_USER_REGISTER帧: len=%d =======\n", len);
    Serial.printf("用户注册数据: ");
    for (int i = 0; i < len && i < 64; i++) {
      Serial.printf("%02X ", data[i]);
    }
    Serial.println();

    // 尝试解析用户信息（格式：user_id|employee_no|name|rfid_uid|finger_id）
    char userData[128] = {0};
    memcpy(userData, data, (len < 127) ? len : 127);
    userData[127] = '\0';
    Serial.printf("用户注册字符串: %s\n", userData);

    // 按 | 分割字段并构造标准的用户添加JSON
    char temp[128] = {0};
    memcpy(temp, data, (len < 127) ? len : 127);
    temp[127] = '\0';
    ...
    String json = "{\"device_id\":\"" + DEVICE_ID + "\",\"frame_type\":" + String((int)TYPE_ADD_USER);
    json += ",\"user_id\":" + String(parts[0] != NULL ? parts[0] : "0");
    json += ",\"employee_no\":\"" + String(parts[1] != NULL ? parts[1] : "") + "\"";
    json += ",\"name\":\"" + String(parts[2] != NULL ? parts[2] : "") + "\"";
    json += ",\"rfid_uid\":\"" + String(parts[3] != NULL ? parts[3] : "") + "\"";
    json += ",\"finger_id\":" + String(parts[4] != NULL ? parts[4] : "0");
    json += "}";
    
    Serial.printf("构建的用户注册JSON: %s\n", json.c_str());
    Serial.printf("上报用户注册MQTT: %s\n", json.c_str());
    Serial.printf("MQTT主题: %s\n", MQTT_TOPIC_PUB.c_str());
    Serial.printf("MQTT连接状态: %s\n", mqttClient.connected() ? "已连接" : "未连接");

    if (mqttClient.publish(MQTT_TOPIC_PUB.c_str(), json.c_str())) {
      Serial.println("用户注册MQTT发布成功");
    } else {
      Serial.println("用户注册MQTT发布失败");
      Serial.printf("MQTT状态码: %d\n", mqttClient.state());
    }
    Serial.println("======= TYPE_USER_REGISTER处理完成 =======\n");
  }
  else if (type == TYPE_RESTART_ESP) {
    Serial.println("收到重启ESP8266命令，正在重启...");
    delay(100);
    ESP.restart();  // ESP8266重启命令
  }
  else {
    Serial.printf("收到未知帧类型: 0x%02X, len=%d\n", type, len);
    Serial.printf("帧数据: ");
    for (int i = 0; i < len && i < 32; i++) {
      Serial.printf("%02X ", data[i]);
    }
...
void mqttReconnect() {
  ...
  while (!mqttClient.connected()) {
    ...
    if (mqttClient.connect(DEVICE_ID.c_str(), MQTT_USER, MQTT_PWD)) {
      ...
      if (mqttClient.subscribe(MQTT_TOPIC_SUB.c_str())) {
        Serial.println("订阅成功");
        digitalWrite(LED_BUILTIN, LOW);
      } else {
        Serial.println("订阅失败");
...
void handleMqttCmd(char* topic, byte* payload, unsigned int length) {
  ...
  Serial.printf("新增用户指令发送给STM32: %.*s\n", length, cmd);
}
...
String frame2Json(uint8_t type, uint8_t* data, uint8_t len) {
  ...
  } else if (type == TYPE_ADD_USER && len > 0 && data != NULL) {
    // 解析用户信息：格式为 "user_id|employee_no|name|rfid_uid|finger_id"
    char temp[128] = {0};
    memcpy(temp, data, (len < 127) ? len : 127);
    temp[127] = '\0';
    // 不调用 cleanString，以保留中文字符
    // 按 | 分割字段（使用 strtok_r 避免与 MQTT 回调的 strtok 冲突）
    char* parts[5] = {NULL, NULL, NULL, NULL, NULL};
    char* saveptr;
    char* token = strtok_r(temp, "|", &saveptr);
    for (int i = 0; i < 5 && token != NULL; i++) {
